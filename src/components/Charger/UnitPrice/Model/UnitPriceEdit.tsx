...
interface ModalProps {
  ...
  unitPriceID: number | '';
  setUnitPriceID: Dispatch<SetStateAction<number | ''>>;
}
const UnitPriceEdit = ({
  state,
  setState,
  ...
  const { loading, error, data, getData } = useGetDataWtTrigger<IUnitPrice>();

  useEffect(() => {
    // bannereventId를 받아오면 api 호출
    if (unitPriceID !== '') {
      void getData({
        url: `/v1/unit-price-set/${unitPriceID}`,
      });
    }
  }, [unitPriceID]);
  ...
  useEffect(() => {
    if (data !== null) {
      // 모달 열림
      form.setFieldsValue({
        unitPriceSetName: data?.unitPriceSetName,
        ...
        unitPrice8: data?.unitPrice8,
        unitPrice9: data?.unitPrice9,
        unitPrice10: data?.unitPrice10,
        unitPrice11: data?.unitPrice11,
        unitPrice12: data?.unitPrice12,
        unitPrice13: data?.unitPrice13,
        unitPrice14: data?.unitPrice14,
        unitPrice15: data?.unitPrice15,
        unitPrice16: data?.unitPrice16,
        unitPrice17: data?.unitPrice17,
        unitPrice18: data?.unitPrice18,
        unitPrice19: data?.unitPrice19,
        unitPrice20: data?.unitPrice20,
        unitPrice21: data?.unitPrice21,
        unitPrice22: data?.unitPrice22,
        unitPrice23: data?.unitPrice23,
        unitPrice24: data?.unitPrice24,
      });
      setIsEditOpen(true);
    }
  }, [data]);
  ...
  async function onFinish(values: any) {
    ...
    await putApi(
      {
        url: `/unit-price-set/${unitPriceID}`,
        data: addData,
      },
      setState,
    );
  ...
  // const validateMaxLenght = (e: any) => {
  //   console.log(e);
  // };

  return (
    <>
      <Modal open={isEditOpen} title="단가테이블" close={handleCloseModal}>
        <StyledForm
          form={form}
          name="unitPriceEdit"
          colon={false}
