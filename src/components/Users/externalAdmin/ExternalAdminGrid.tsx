...
import ExternalAdminGridHeader from './ExternalAdminGridHeader';
import ExternalAdminGridTable from './ExternalAdminGridTable';
...
const HDOAdminGrid = () => {
  ...
  const { loading, totalCount, data, refetch } = useGetListWt({
    url: `/v1/web/users?userType=org&rpp=${queryState.rpp}&page=${queryState.page}&odby=${queryState.odby}&select=${queryState.searchKey}&search=${queryState.searchVal}&startDate=${queryState.startDate}&endDate=${queryState.endDate}&org=${queryState.category}`,
  });
  useEffect(() => {
    if (state.isSuccess) {
      setAlertModal({
  ...
  return (
    <DefaultDiv>
      <ExternalAdminGridHeader
        queryState={queryState}
        setQueryState={setQueryState}
        refetch={search}
        reload={reload}
      />
      <ExternalAdminGridTable
        loading={loading}
        data={data}
        state={state}
        setState={setState}
