...
const ClientAdminWaitGrid = ({
  ...
  useEffect(() => {
    if (clientData !== '') {
      void getData({
        ...
        url: `/v1/web/orgs/${clientData.id}/users/external/request`,
      });
      setIsModalOpen(true);
    }
  }, [clientData]);
  ...
  // 그리드 객체
  const clientAdminWaitGrid = useRef<AUIGrid>(null);
  ...
  useEffect(() => {
    if (context.selectedIndex === '2') {
      const grid = clientAdminWaitGrid.current as AUIGrid;
      grid.resize();
    }
  ...
  return (
    <DefaultDiv>
      ...
      <AUIGridContainer style={{ height: 200 }}>
        <AUIGrid
          ref={clientAdminWaitGrid}
          columnLayout={columnLayout}
          gridProps={gridProps}
        />
      </AUIGridContainer>
      <ClientAdminEdit
        name="clien-admin-wait-edit"
        state={state}
        setState={setState}
        clientAdminData={clientAdminData}
        setClientAdminData={setClientAdminData}
      />
    </DefaultDiv>
  );
};

export default ClientAdminWaitGrid;
