...
import ClientAdminRegister from './ClientAdminRegister';
import ClientAdminEdit from './ClientAdminEdit';
import { useRecoilState } from 'recoil';
import { alertModalState } from 'recoil/modalState';

const ClientAdminGrid = ({
  clientData,
  state,
  setState,
  setIsModalOpen,
  ...
  useEffect(() => {
    if (clientData !== '') {
      void getData({
        ...
        url: `/v1/web/orgs/${clientData.id}/users`,
      });
      setIsModalOpen(true);
    }
  }, [clientData]);
      ...
      <GridHeader between>
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <LabelWrap label="소속명">
            <StyledInput
              readOnly
              value={clientData !== '' ? clientData.name : '에러'}
            />
          </LabelWrap>
        </div>
        <ClientAdminRegister
...
export default ClientAdminGrid;
