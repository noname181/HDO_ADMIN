...
const ContractorAdminEdit = ({
  contractorAdminData,
  setContractorAdminData,
  state,
  setState,
}: any) => {
  ...
  useEffect(() => {
    if (contractorAdminData !== '') {
      getPermissions();
      void getDetail({
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        url: `/v1/web/users/${contractorAdminData.id}`,
      });
    }
  }, [contractorAdminData]);
  ...
  async function onFinish(values: any) {
    if (contractorAdminData !== '') {
      ...
      await putApi(
        {
          url: `/v1/web/users/${contractorAdminData.id as string}`,
          data: editData,
        },
        setState,
      );
  ...
  return (
    <Modal open={isModalOpen} title="관리자 계정 수정" close={handleCloseModal}>
      <StyledForm form={form} name="contractor-user-edit" colon={false}>
        <StyledFormItem
          name="status"
          label="이용유무"
          rules={[
...
export default ContractorAdminEdit;
