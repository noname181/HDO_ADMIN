...
const ChargerHistoryEdit = ({
  ...
  const [dataType10, setDataType10] = useState<any>(null);
  ...
  const calculateRefundAmount = async (paymentHistory: any) => {
    if (!paymentHistory?.paymentNotification?.cno) {
      return;
    }
    // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
    const url = `/v1/payment-notifications?cno=${paymentHistory?.paymentNotification?.cno}`;
    ...
    axios(url, {
      ...
      .then((result) => {
        ...
        setpgCno(paymentHistory?.paymentNotification?.cno);
  ...
  useEffect(() => {
    if (payHisData && isConsultantPayHistory) {
      void calculateRefundAmount(payHisData);
    }
  }, [payHisData]);
          ...
          <CPHContainer>
            ...
            <table className="nl-tbl-detail">
              <tbody>
                <tr>
                  <th>번호</th>
                  <td>{payHisData?.auth_no ?? dataType10?.auth_no}</td>
                  <th>부문</th>
                  <td>{payHisData?.chargingStationUseLog?.org?.areaName}</td>
                  <th>지사</th>
                  <td>{payHisData?.chargingStationUseLog?.org?.branchName}</td>
                </tr>
                <tr>
                  <th>구분</th>
                  <td>
                    {category(payHisData?.chargingStationUseLog?.org?.category)}
                  </td>
                  <th>충전소명</th>
                  <td>{payHisData?.chargingStationUseLog?.chgs_name}</td>
                  <th>충전기 ID</th>
                  <td>{payHisData?.chargerUseLog?.chg_charger_id}</td>
                </tr>
                <tr>
                  <th>회원구분</th>
                  <td>
                    {payHisData?.useType
                      ? payHisData?.useType === 'CREDIT'
                        ? '비회원'
                        : '회원'
                      : ''}
                  </td>
                  ...
                  <th>사용자</th>
                  <td>
                    {payHisData?.userNew?.name}
                    {payHisData?.userNew?.accountId && ' ( '}
                    {payHisData?.userNew?.accountId}
                    {payHisData?.userNew?.accountId && ' )'}
                  </td>
                ...
                <tr>
                  <th>적용단가</th>
                  <td>
                    {payHisData?.appliedUnitPrice &&
                      new Intl.NumberFormat('en-US').format(
                        payHisData?.appliedUnitPrice,
                      ) + '원'}
                  </td>
                  <th>인증방법</th>
                  <td>{payHisData?.useType}</td>
                  <th>예약 충전 여부</th>
                  <td></td>
                </tr>
                <tr>
                  <th>충전 요청</th>
                  <td>
                    {payHisData?.desired_kwh
                      ? new Intl.NumberFormat('en-US').format(
                          payHisData?.desired_kwh * 0.001,
                        ) + 'kWh'
                      : ''}
                  </td>
                  <th>충전량</th>
                  <td>
                    {payHisData?.cl_kwh
                      ? `${new Intl.NumberFormat('en-US').format(
                          payHisData?.cl_kwh * 0.001,
                        )}kWh`
                      : ''}
                  </td>
                  {/* <th>실 충전량</th>
                <td>
                  {Number(payHisData?.chargingLogs?.cl_kwh) +
                    Number(payHisData?.chargingLogs?.ignored_kwh) || ''}
                  {payHisData?.chargingLogs?.cl_kwh ||
                  payHisData?.chargingLogs?.ignored_kwh
                    ? 'kWh'
                    : ''}
                </td> */}
                  <th></th>
            ...
            <table className="nl-tbl-detail">
              <tbody>
                <tr>
                  <th>카드사</th>
                  <td>{payHisData?.issuer_nm_noti10}</td>
                  <th>카드번호</th>
                  <td>{payHisData?.card_no_noti10}</td>
                </tr>
              </tbody>
            </table>
            {/* table vertical */}
            <table className="nl-tbl-vertical">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td colSpan={4} style={{ fontSize: 15 }}>
                    {new Intl.NumberFormat('en-US').format(calculatedAmount)}
                  </td>
                </tr>
              </tbody>
            </table>
...
export default ChargerHistoryEdit;
