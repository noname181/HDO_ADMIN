...
import ChargerDiagnosticDetail from './ChargerDiagnosticDetail';

export const ChargerDiagnosticGridTable = ({
  loading,
  data,
  setSendId,
  sendId,
  ...
  const requestAddData = async () => {
    ...
    const url = `/charger-update/tm?rpp=${queryState.rpp}&page=${queryState.page}&odby=${queryState.odby}&searchKey=${queryState.searchKey}&searchVal=${queryState.searchVal}&startDate=${queryState.startDate}&endDate=${queryState.endDate}&update=${queryState.update}&clause_version=${queryState.clause_version}`;
    ...
    if (!loading && data !== null) {
      ...
      grid.bind(
        ...
        (event: IGrid.CellDoubleClickEvent) => {
          setDiagnosticId(event.item.chg_id);
          showDetail();
        },
      );
      grid.bind(
    ...
    <>
      <GridContainer height="calc(100vh - 15.7rem)">
        ...
        >
          ...
          >
            *“충전가능” 상태일때만 진단정보를 충전기로부터 받아올 수 있습니다.
          </span>
        </GridButton>
