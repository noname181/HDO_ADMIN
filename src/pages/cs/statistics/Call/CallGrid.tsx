...
import CallGridHeader from './CallGridHeader';
import CallGridTable from './CallGridTable';
import { useEffect, useState } from 'react';
import { type UseGetListResponse } from 'interfaces/IUseGetData';
import { type StateInterface, type callLog } from 'interfaces/ICommon';
...
const CallGrid = () => {
  ...
  const { loading, data, refetch, totalCount }: UseGetListResponse<callLog> =
    useGetListWt({
      url: `/v1/web/cs-callList?rpp=${queryState.rpp}&page=${queryState.page}&startDate=${queryState.startDate}&endDate=${queryState.endDate}&gubun=callLogList`,
    });
  const search = () => {
    setQueryState({
      ...queryState,
